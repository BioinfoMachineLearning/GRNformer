...
class PretrainedEmbeddingModel(nn.Module):
    ...
    def forward(self, x):
        with torch.no_grad():  # Disable gradient calculations
            embeddings = self.pretrained_model(x)  # Extract embeddings
        return embeddings

class GRNFormerLinkPred(pl.LightningModule):
    def __init__(self, codoformer,learning_rate=1e-3,node_dim=NODES_DIM,out_chan=OUT_CH,num_heads=NUM_HEADS,edge_dim=EDGE_DIM,encoder_layers=ENCODE_LAYERS, **model_kwargs):
        super().__init__()
    ...
    def configure_optimizers(self):
        ...
        #               steps_per_epoch=100, 
         ##              pct_start=0.3,  # 30% warm-up phase
          ...
          #             div_factor=25.0,  # Divides initial LR by 25
          #             final_div_factor=1e4)  # Final learning rate is max_lr / 1e4
        ...
        #             step_size_up=2000,  # Number of iterations to increase the LR
...
def train_GRNFormerLinkPred():
    ...
    checkpoint_path = os.path.abspath("Pretrained_GeneTranscoder/GRNFormer_beeline_epoch=01_valid_loss=0.000531.ckpt")
    ...
    model = GRNFormerLinkPred(learning_rate=1e-3,codoformer=embedding_model)
    
    trainer = pl.Trainer.from_argparse_args(args)
    

    checkpoint_callback = ModelCheckpoint(monitor='valid_loss', save_top_k=10, dirpath=DATASET_DIR+"/Trainings/"+args.save_dir, filename='GRNFormer_beeline_{epoch:02d}_{valid_loss:6f}')
...
# Load the checkpoint
    ...
    trainer.fit(model, train_loader, valid_loader)#,ckpt_path="GRNFormerCodoformer_embedNegsampling_CycleLR/GRNFormer_beeline_epoch=20_valid_loss=0.690517.ckpt")
