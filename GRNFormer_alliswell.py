...
from GRNFormerModels import EdgeTransformerEncoder_new,DeepVGAE
...
CHECKPOINT_PATH = f"{DATASET_DIR}/GRNformerV5_VSC_Newly_labelled"
...
class GRNFormerLinkPred(pl.LightningModule):
    def __init__(self, learning_rate=1e-4,node_dim=NODES_DIM,out_chan=OUT_CH,num_heads=NUM_HEADS,edge_dim=EDGE_DIM,encoder_layers=ENCODE_LAYERS, **model_kwargs):
        ...
        self.model = VGAE(encoder=EdgeTransformerEncoder_new(node_dim, out_chan,num_head=num_heads,edge_dim=edge_dim,num_layers=encoder_layers),decoder=InnerProductDecoder())
    ...
    def training_step(self,batch,batch_idx):
        ...
        grn_pred_edge = self.forward(batch_data,batch_edges,batch_edge_attr)
