    ...
    def process(self):
        ...
        for tf_gene in self.tf_genes:
            if tf_gene in gene_indices:
                ...
                if len(first_hop_neighbors) + 1 >= 50:
                    subgraph_node_idx = torch.cat([torch.tensor([tf_idx]), first_hop_neighbors[:49]])
                else:
                    remaining_nodes = 50 - (len(first_hop_neighbors) + 1)
