...
import random
import DatasetwithTFcenter as dt
...
def perturb_features(node_features, noise_level=0.01):
    noise = torch.randn_like(node_features) * noise_level
    return node_features + noise
def create_subgraph(edge_index, node_features, central_node, num_neighbors=499):
    # Identify the 499 nearest neighbors for the central node
    nbrs = NearestNeighbors(n_neighbors=num_neighbors+1, algorithm='auto').fit(node_features)
    distances, indices = nbrs.kneighbors(node_features)
    neighbors = indices[central_node][1:]  # exclude the central node itself
    
    sub_nodes = np.concatenate(([central_node], neighbors))
    sub_nodes.sort()
    
    # Create a subgraph with the central node and its neighbors
    mask = np.isin(edge_index[0], sub_nodes) & np.isin(edge_index[1], sub_nodes)
    sub_edge_index = edge_index[:, mask]
    
    # Remap nodes to have consecutive node indices in the subgraph
    node_map = {node: idx for idx, node in enumerate(sub_nodes)}
    sub_edge_index = sub_edge_index.apply_(lambda x: node_map[x.item()])
    
    # Extract the node features for the subgraph
    sub_node_features = node_features[sub_nodes]
    
    return sub_edge_index, sub_node_features, central_node

def create_dataset(edge_index, node_features, num_nodes):
    data_list = []
    for i in range(num_nodes):
        sub_edge_index, sub_node_features, central_node = create_subgraph(edge_index, node_features, i)
        data = Data(x=sub_node_features, edge_index=sub_edge_index, y=torch.tensor([central_node]))
        data_list.append(data)
    return data_list

class GCENgraph(Dataset):
    def __init__(self, root, transform=None, target_transform=None,stage="train",num_sam = 100):
        self.root = root
        self.transform = transform
        self.target_transform = target_transform
        ...
        tf_genes = read_csv("/home/aghktb/GRN/GRNformer/Data/sc-RNA-seq/hHep/TFHumans.csv",header=None)[0].to_list()
        ...
        for data in dataset:
                ...
                #print(torch.any(gsum_ten.isnan()))
                #gcengraph = construct_networkx(Coexp_mat)
                #print(gcengraph.edge_index.shape)
                ...
                pert_exp = perturb_features(zscores_cells)
                node_att = torch.column_stack((is_tf,pert_exp.t()))
                targ_grn_activated = activate_grn(expr_mat,mean.T,Target_grn,is_tf.T)
                ...
                for i in range(self.num_sam):
                            ...
                            sam_edge_index = negative_sampling(subg.edge_index, subg.num_nodes)
