...
class GCENData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        Exp_label_pth = os.path.dirname(Exp_data)+"/Filtered--network.csv"
        ...
        minm_acrossgenes = min_max(exp_mat_forz)
        ...
        minmax_exp_acrosscell = min_max_dim0(exp_mat_df)
...
class GCENgraph(Dataset):
    def __init__(self, root, transform=None, target_transform=None):
        ...
        for gr in self.GCENset:
              num_samples = max(100, len(gr) // 100)
              subdata = RandomNodeLoader(data=gr,num_parts=10)
              self.SubaGraphs.extend(subdata)
