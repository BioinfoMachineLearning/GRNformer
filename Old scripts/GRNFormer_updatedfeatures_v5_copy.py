...
from GRNFormerNewModels import EdgeTransformerEncoder_tcn
...
class GRNFormerLinkPred(pl.LightningModule):
    ...
    def forward(self, data_sampled,edge_sampled,edge_attr):
        ...
        grnedges = self.model.decoder.forward_all(grnedges_lat,sigmoid=True)
        return grnedges_lat,grnedges
    ...
    def training_step(self,batch,batch_idx):
        ...
        batch_edge_attr = torch.transpose(batch[5],0,1).cuda()
        batch_y = batch[4].squeeze(0).cuda()
        grn_pred_edge,grn_logit = self.forward(batch_data,batch_edges,batch_edge_attr)
        ...
        metrics = self.train_metrics(grn_logit.view(-1),batch_y.view(-1))
    ...
    def validation_step(self,batch,batch_idx):
        ...
        metrics = self.valid_metrics(grn_logit.view(-1),batch_y.view(-1))
        self.log_dict(metrics)
    ...
    def test_step(self,batch, batch_idx,dataloader_idx):
        ...
        loss = self.loss_fn(grn_pred_edge,batch_targ_pos,batch_targ_neg)
        loss = loss + (1 / len(batch_data)) * self.kl()
        metrics = self.test_metrics(grn_logit.view(-1),batch_y.view(-1))
